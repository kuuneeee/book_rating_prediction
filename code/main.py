...
def main(args):
    ...
    if args.MODEL=='FM':
        ...
        model = DeepCrossNetworkModel(args, data, idx)
    elif args.MODEL=='CNN_FM':
...
if __name__ == "__main__":
    ...
    ############### TRAINING OPTION
    arg('--BATCH_SIZE', type=int, default=2048, help='Batch size를 조정할 수 있습니다.')    # 1024
    arg('--EPOCHS', type=int, default=10, help='Epoch 수를 조정할 수 있습니다.')            # 10
    arg('--LR', type=float, default=2e-3, help='Learning Rate를 조정할 수 있습니다.')       # 1e-3 6e-3
    arg('--WEIGHT_DECAY', type=float, default=1e-7, help='Adam optimizer에서 정규화에 사용하는 값을 조정할 수 있습니다.') #1e-6 1e-5
    ...
    ############### FFM 512 1.5e-3 10
    arg('--FFM_EMBED_DIM', type=int, default=32, help='FFM에서 embedding시킬 차원을 조정할 수 있습니다.')    # 16



    ############### NCF 512 1e-3 1
    arg('--NCF_EMBED_DIM', type=int, default=64, help='NCF에서 embedding시킬 차원을 조정할 수 있습니다.')       # 16
    arg('--NCF_MLP_DIMS', type=list, default=(1024, 1024), help='NCF에서 MLP Network의 차원을 조정할 수 있습니다.') # 16 16
    arg('--NCF_DROPOUT', type=float, default=0.2, help='NCF에서 Dropout rate를 조정할 수 있습니다.')

    ############### WDN 512 1e-3 1
    arg('--WDN_EMBED_DIM', type=int, default=64, help='WDN에서 embedding시킬 차원을 조정할 수 있습니다.')
    arg('--WDN_MLP_DIMS', type=list, default=(704, 704), help='WDN에서 MLP Network의 차원을 조정할 수 있습니다.')
    ...
    arg('--DCN_MLP_DIMS', type=list, default=(1152, 1152), help='DCN에서 MLP Network의 차원을 조정할 수 있습니다.')
    arg('--DCN_DROPOUT', type=float, default=0.2, help='DCN에서 Dropout rate를 조정할 수 있습니다.')
    arg('--DCN_NUM_LAYERS', type=int, default=10, help='DCN에서 Cross Network의 레이어 수를 조정할 수 있습니다.')
    arg('--DCN_MLP_NUM_LAYERS', type=int, default=10, help='DCN에서 MLP Network의 레이어 수를 조정할 수 있습니다.')



    ############### CNN_FM
    arg('--CNN_FM_EMBED_DIM', type=int, default=128, help='CNN_FM에서 user와 item에 대한 embedding시킬 차원을 조정할 수 있습니다.')
    arg('--CNN_FM_LATENT_DIM', type=int, default=128, help='CNN_FM에서 user/item/image에 대한 latent 차원을 조정할 수 있습니다.')
