...
class MultiLayerPerceptron(nn.Module):
    ...
    def __init__(self, input_dim, embed_dims, dropout, output_layer=True):
        ...
        self.mlp1 = torch.nn.Sequential(*layers)
...
class DCN_MultiLayerPerceptron(nn.Module):
    
    def __init__(self, input_dim, embed_dims, dropout, mlp_num_layers, output_layer=True):
        super().__init__()
        # layers = list()
        # for embed_dim in embed_dims:
        #     layers.append(torch.nn.Linear(input_dim, embed_dim))
        ...
        self.mlp_num_layer = mlp_num_layers

        layers = list()
        if self.mlp_num_layer:
...
class _FeaturesEmbedding(nn.Module):
...
class _CNN_FM(torch.nn.Module):
    def __init__(self, field_dims, embed_dim, latent_dim):
        super().__init__()
        self.embed_dim = len(field_dims[0:2]) * embed_dim
        self.embedding = _FeaturesEmbedding(field_dims[0:2], self.embed_dim)
        self.cnn = CNN_Base()
        self.fm = FactorizationMachine_v(
                                         input_dim=(self.embed_dim * 2) + (12 * 4 * 4),
                                         latent_dim=latent_dim,
                                         )

    def forward(self, x):
        user_isbn_vector, img_vector = x[0], x[1]
        user_isbn_feature = self.embedding(user_isbn_vector[0:2])
...
class _DeepCrossNetworkModel(nn.Module):
    ...
    def __init__(self, field_dims: np.ndarray, embed_dim: int, num_layers: int, mlp_num_layers: int, mlp_dims: tuple, idx:dict, args, dropout: float):
        super().__init__()
        # print(field_dims)
        self.embedding = FeaturesEmbedding(field_dims, embed_dim)
        self.embed_output_dim = len(field_dims) * embed_dim
    ...
    def forward(self, x: torch.Tensor):
        """
        :param x: Long tensor of size ``(batch_size, num_fields)`` 2048 11
        """
        ...
        ########################## classification
        # x_ = self.cd_linear_s(x_out)
        # # x_ = self.cd_linear_p(x_out)
        # p = F.softmax(x_)
        ########################## classification
