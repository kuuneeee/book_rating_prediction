...
def context_data_load(args):
    ...
    # train_data = pd.read_csv(args.PREPROCESSED_PATH + f'{args.MODEL}_train_preprocessed_data.csv')
    # test_data = pd.read_csv(args.PREPROCESSED_PATH + f'{args.MODEL}_test_preprocessed_data.csv')
    # mean_data = pd.read_csv(args.PREPROCESSED_PATH + f'{args.MODEL}_user_mean_preprocessed_data.csv')
    ...
    #                        len(idx['user_mean2idx']), len(idx['isbn_mean2idx']),
    #                        len(idx['publisher2idx']), len(idx['language2idx']), len(idx['author2idx'])], dtype=np.uint32)

    field_dims = np.array([len(user2idx), len(isbn2idx), 6, len(idx['user_mean2idx']), len(idx['isbn_mean2idx']),
