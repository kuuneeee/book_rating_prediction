...
def process_context_data(users, books, ratings1, ratings2, user2idx):
    ...
    ### rating
    # user_id_ = ratings1['user_id'].unique()
    # ratings1['mean'] = 0
    # for id in user_id_:
    #     user_id_rating_mean = ratings1[ratings1['user_id']==id].groupby('user_id')['rating'].mean()
    #     # mean_rating.append(user_id_rating_mean)
    #     # user_id_rating.append(id)
    #     user_id_rating = ratings1[ratings1['user_id']==id]['rating']
    #     user_id_idx = ratings1[ratings1['user_id']==id]['rating'].index
    #     for idx, rating in zip(user_id_idx, user_id_rating):
    #         ratings1.loc[ratings1[ratings1['user_id'].index==idx]['user_id'].index, 'rating'] = rating - float(user_id_rating_mean)
    #         ratings1.loc[ratings1[ratings1['user_id'].index==idx]['user_id'].index, 'mean'] = float(user_id_rating_mean)
...
def context_data_load(args):
    ...
    # language
    lan = {0:'en', 1:'en', 2:'fr', 3:'de', 4:'ja', 5:'ru', 6:'it', 7:'zh-CN', 8:'es', 9:'nl', 'B':'da'}
    not_int = []
    for i in range(len(books['isbn'])):
        idx = books['isbn'][i][0]
        if idx in '0123456789':
            books.loc[ books[books['isbn']==books['isbn'][i]].index, 'language'] = lan[int(idx)]
            print(books.loc[books[books['isbn']==books['isbn'][i]].index,'language'], idx, lan[int(idx)])
        elif idx == 'B':
            books.loc[ books[books['isbn']==books['isbn'][i]].index, 'language'] = lan[idx]
            print(books.loc[books[books['isbn']==books['isbn'][i]].index,'language'], idx, lan[idx])

    # print(books['isbn','language'])
    ...
    idx, context_train, context_test = process_context_data(users, books, train, test, user2idx)
    
    field_dims = np.array([len(user2idx), len(isbn2idx), 6, len(idx['loc_city2idx']), len(idx['loc_state2idx']),
                            len(idx['loc_country2idx']), len(idx['category2idx']), len(idx['publisher2idx']), len(idx['language2idx']), len(idx['author2idx'])], dtype=np.uint32)
...
def context_data_split(args, data):
    X_train, X_valid, y_train, y_valid = train_test_split(
                                                        data['train'].drop(['rating', 'mean'], axis=1),
                                                        data['train']['rating'],
